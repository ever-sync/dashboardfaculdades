-- ============================================
-- Script para criar a tabela configuracoes_globais
...
-- Política: Permitir leitura para usuários autenticados (mas valores sensíveis serão mascarados)
DROP POLICY IF EXISTS "configuracoes_globais_authenticated_read" ON public.configuracoes_globais;
CREATE POLICY "configuracoes_globais_authenticated_read" ON public.configuracoes_globais
  FOR SELECT
  TO authenticated
  USING (true);

-- Política: Permitir escrita para usuários autenticados
DROP POLICY IF EXISTS "configuracoes_globais_authenticated_write" ON public.configuracoes_globais;
CREATE POLICY "configuracoes_globais_authenticated_write" ON public.configuracoes_globais
  FOR ALL
  TO authenticated
  USING (true)
  WITH CHECK (true);
